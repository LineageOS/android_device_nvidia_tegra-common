...
do_insmod /vendor/lib/modules/pwm-tegra.ko
...
if [ "$hardwareName" = "abca" ]; then
     do_insmod /odm/lib/modules/fts.ko
fi
...
# Recovery load is loading vendor accessory modules on boot in recovery mode
recovery_load() {
/vendor/bin/log -t "$scriptName" -p i "Loading vendor accessory modules for recovery mode started"
do_insmod /vendor/lib/modules/hid-nvidia-blake.ko
do_insmod /vendor/lib/modules/hid-jarvis-remote.ko
/vendor/bin/log -t "$scriptName" -p i "Loading vendor accessory modules for recovery mode completed"

/vendor/bin/log -t "$scriptName" -p i "Loading vendor usb storage module for recovery mode started"
do_insmod /vendor/lib/modules/usb-storage.ko
/vendor/bin/log -t "$scriptName" -p i "Loading vendor usb storage module for recovery mode completed"

/vendor/bin/setprop vendor.lkm.completed 1
}
