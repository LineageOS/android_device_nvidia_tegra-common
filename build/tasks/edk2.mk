...
LIBAVB_PATH        := $(BUILD_TOP)/external/avb
...
ifeq ($(EDK2_TOOLCHAIN),CLANGDWARF)
TOOLCHAIN_VARS := CLANG_HOST_BIN=$(abspath $(dir $(KERNEL_MAKE_CMD)))/ CC=$(TARGET_KERNEL_CLANG_PATH)/bin/clang CXX=$(TARGET_KERNEL_CLANG_PATH)/bin/clang++ AS=$(TARGET_KERNEL_CLANG_PATH)/bin/clang AR=$(TARGET_KERNEL_CLANG_PATH)/bin/llvm-ar LD=$(TARGET_KERNEL_CLANG_PATH)/bin/llvm-ld CLANG_BIN=$(TARGET_KERNEL_CLANG_PATH)/bin/ CLANGDWARF_BIN=$(TARGET_KERNEL_CLANG_PATH)/bin/ EXTRA_LDFLAGS="-fuse-ld=lld" CLANGDWARF_$(EDK2_ARCH)_PREFIX=$(TARGET_KERNEL_CLANG_PATH)/bin/llvm-
else
TOOLCHAIN_VARS := CLANG_HOST_BIN=$(abspath $(dir $(KERNEL_MAKE_CMD)))/ CC=$(TARGET_KERNEL_CLANG_PATH)/bin/clang CXX=$(TARGET_KERNEL_CLANG_PATH)/bin/clang++ AS=$(TARGET_KERNEL_CLANG_PATH)/bin/clang AR=$(TARGET_KERNEL_CLANG_PATH)/bin/llvm-ar LD=$(TARGET_KERNEL_CLANG_PATH)/bin/llvm-ld CLANG_BIN=$(TARGET_KERNEL_CLANG_PATH)/bin/ GCC_HOST_BIN=$(abspath $(dir $(KERNEL_MAKE_CMD)))/ GCC_$(EDK2_ARCH)_PREFIX=$(KERNEL_TOOLCHAIN)/$(KERNEL_TOOLCHAIN_PREFIX) DTCPP_PREFIX=$(KERNEL_TOOLCHAIN)/$(KERNEL_TOOLCHAIN_PREFIX)
endif
...
$(1): $(DTC_HOST)
	...
	@cp -R $(LIBAVB_PATH) $(dir $1)/edk2-nvidia/Silicon/NVIDIA/Library/AvbLib/libavb
	@export PYTHON_COMMAND=$(BUILD_TOOLS_PATH)/python3 && \
		export WORKSPACE=$(abspath $(dir $1)) && \
		export PACKAGES_PATH=$(abspath $(dir $1))/edk2-nvidia:$(abspath $(dir $1))/edk2:$(abspath $(dir $1))/edk2-non-osi:$(abspath $(dir $1))/edk2-platforms:$(abspath $(dir $1))/edk2-nvidia-non-osi:$(abspath $(dir $1))/edk2-platforms/Features/Intel/OutOfBandManagement && \
		...
		PYTHONPATH=$$$$PYTHONPATH:$(TIANOCORE_PATH)/edk2-nvidia-lineage/Silicon/NVIDIA/scripts/Kconfiglib \
			KCONFIG_CONFIG=$(abspath $(dir $1))/edk2-nvidia/.config \
			$(TIANOCORE_PATH)/edk2-nvidia-lineage/Silicon/NVIDIA/scripts/Kconfiglib/defconfig.py \
			--kconfig $(abspath $(dir $1))/edk2-nvidia/Platform/NVIDIA/Kconfig \
			$(abspath $(TIANOCORE_PATH))/edk2-nvidia-lineage/Platform/NVIDIA/Tegra/DefConfigs/$(TARGET_TEGRA_VERSION)_android.defconfig && \
		sed 's/"//g' $(abspath $(dir $1))/edk2-nvidia/.config > $(abspath $(dir $1))/edk2-nvidia/config.dsc.inc && \
		sed -E -i 's/^CONFIG_([^=]*)=$$$$/# CONFIG_\1 is not set/' $(abspath $(dir $1))/edk2-nvidia/config.dsc.inc && \
		$(abspath $(dir $1))/edk2/BaseTools/BinWrappers/PosixLike/build -q -a $(EDK2_ARCH) -t $(EDK2_TOOLCHAIN) \
			-p $(abspath $(dir $1))/edk2-nvidia/Platform/NVIDIA/NVIDIA.common.dsc -b $(EDK2_BUILD_TYPE) \
			$3 -DEDK2_PKCS7_INC=$(EDK2_PKCS7_INC) -DBUILD_PROJECT_TYPE=EDK2 -DBUILD_DATE_TIME="$(shell date '+%Y-%m-%d')" -DBUILD_NAME="LineageTegra" \
			-DBUILD_GUID="$(shell grep PLATFORM_GUID $(TIANOCORE_PATH)/edk2-nvidia-lineage/Platform/NVIDIA/Tegra/DefConfigs/$(TARGET_TEGRA_VERSION)_android.defconfig |cut -d\" -f 2)" \
...
$(INSTALLED_TIANOCORE_DTBO_TARGETS): $(PRODUCT_OUT)/tianocore.bin
	...
	cp $(@F:%.dtbo=$(call intermediates-dir-for,EXECUTABLES,tianocore)/Build/DeviceTree/$(EDK2_BUILD_TYPE)_$(EDK2_TOOLCHAIN)/$(EDK2_ARCH)/Silicon/NVIDIA/Tegra/DeviceTree/DeviceTree/OUTPUT/%.dtb) $@
endif
